...
### COMMENTS
	...
	nColVars_list = range(nColVars)
        ...
        np.savetxt(system_descriptor+'.covariance_matrix.dat',covariance_matrix,delimiter=' ',fmt='%f')
        np.savetxt(system_descriptor+'.correlation_matrix.dat',correlation_matrix,delimiter=' ',fmt='%f')
...
### COMMENTS
	...
	nFrames_range = range(nFrames)
        ...
        with open(system_descriptor+'.cluster_validation.dat','w') as W, open(system_descriptor+'.cluster_centers.dat','w') as X:
			...
			nClusters_range = range(nClusters)
                        #-------------------------------------
                	# Initialize the clusterer with desired kwargs
                        ...
                        for i in nClusters_range:
				for j in nClusters_range[i+1:]:
                                        cluster_dist_matrix[i,j] = (np.sum(np.square(cluster_centers[i] - cluster_centers[j])))**0.5
